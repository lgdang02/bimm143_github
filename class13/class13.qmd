...
> Q5. Remove any "zero count" genes from out dataset for further analysis
...
The first function we will use from this package sets up input in the particular format that DESeq wants:
...
We can flip the y-axis by adding a minus sign. This will make it easier to intepret
...
> Q8. Make a ggplot volcano plot with colors and lines as annotation along with nice axis label
...
We first need to add gene symbols (e.g. HBB) so we know what genes we are dealing with. We need to "translate" between ENSEMBLE ids that we have in the rownamess of `res`.
...
![The Asthma pathway overlaps with our differentially exppresed genes](hsa05310.pathview.png)
